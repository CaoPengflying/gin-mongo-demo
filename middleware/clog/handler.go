...
type Handler interface {
	...
	LogWith(context.Context, Level, string, map[string]interface{})
	Close() error
}
...
func (hs *Handlers) LogWith(ctx context.Context, l Level, msg string, m map[string]interface{}) {
	for _, h := range hs.handlers {
		h.LogWith(ctx, l, msg, m)
	}
}
